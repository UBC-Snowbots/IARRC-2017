...
void IPMFilterNode::filteredImageCallBack(const sensor_msgs::ImageConstPtr &msg) {
    if (!receivedFirstImage) {
        ...
        SB_getParam(private_nh, "image_height", image_height, (int) msg->height);
        ipmFilter = new IPMFilter(ipm_base_width, ipm_top_width, ipm_base_displacement,
                                  ipm_top_displacement, image_height, image_width);
    }
